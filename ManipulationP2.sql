...
-- Customer Manipulation Manipulation!!

-- 1
SELECT Country, COUNT(*) AS `# of Customers` FROM Customer GROUP BY Country HAVING Country LIKE 'F%';

-- 2
SELECT Country, AVG(ContactAge) AS `AVG(ContactAge)` FROM Customer GROUP BY Country HAVING AVG(ContactAge) > 50 ORDER BY AVG(ContactAge) ASC;

-- 3
SELECT City, COUNT(*) AS `Number of Customers` FROM Customer WHERE CustomerName NOT LIKE '%M%' GROUP BY City HAVING COUNT(*) > 1 ORDER BY COUNT(*) ASC;
...
-- Magic Store Manipulation!!

-- 1
SELECT Category, COUNT(*) AS `Product Count`, MAX(Price) AS `Max Price` FROM Product GROUP BY Category;

-- 2
SELECT Category, COUNT(*) AS `# of Products` FROM Product GROUP BY Category HAVING Category IN (SELECT DISTINCT Category FROM Product WHERE Price > 250);

-- 3
SELECT `Order`.ID AS `Order ID`, SUM(OrderProduct.Quantity) AS `Quantity Ordered` FROM `Order` JOIN OrderProduct ON `Order`.ID = OrderProduct.OrderID GROUP BY `Order`.ID;

-- 4

SELECT Client.FirstName, Client.LastName, COUNT(`Order`.ID) AS `Total Orders` FROM Client LEFT JOIN `Order` ON Client.ID = `Order`.ClientID GROUP BY Client.ID ORDER BY `Total Orders` ASC;